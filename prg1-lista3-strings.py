...
def palindrome(texto):
    ...
    # Retirar os espaços entre as palavras
    texto = texto.strip(' ')

    # Texto normal minúsuclo
    textoLower = texto.lower()

    palavraLimpa = ''

    for caracteres in textoLower:

        if caracteres.isalpha():
            palavraLimpa += caracteres

    return palavraLimpa == palavraLimpa[::-1]
...
def troca_caixa(texto):
    ...
    for letras in texto:
        if letras in "AEIOUaeiou":
            novaPalavra += letras.upper()
        else:
            novaPalavra += letras.lower()
...
def imprime_mes_por_extenso(data):
    ...
    # Dividor o formato dd/mm/aaaa e deixar cada dado separado
    dataDividida = data.split('/')
    dia = dataDividida[0]
    mes = int(dataDividida[1])
    ano = dataDividida[2]

    # Pegar o mês pela posição

    meses = ['janeiro', 'fevereiro', 'março', 'abril', 'maio', 'junho', 'julho', 'agosto', 'setembro', 'outubro', 'novembro', 'dezembro']

    return ("%s de %s de %s" % (dia, meses[mes- 1], ano))
...
def encontra_caracter(texto, caracter_procurado):
    ...
    for caracteres in texto:
        ...
        if caracteres == caracter_procurado:
            return posicao
        else:
            posicao += 1
    return -1
